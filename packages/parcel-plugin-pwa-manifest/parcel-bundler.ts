...
export = (bundler: FullBundler): void => {
  ...
  bundler.on('bundled', async bundle => {
    ...
    let opts = pkg.pwaManifest || pkg['pwa-manifest'];
    ...
    if (env && opts[env.toLowerCase()]) {
      ...
      if (typeof envOpts !== 'object') {
        return err(
          'The specific options for environment "' +
            process.env.NODE_ENV +
            '" must be an object containing the desired parameters.'
        );
      }
      opts = {
        ...otherOpts,
    ...
    const disabled = opts.disabled || opts.disable || false;

    // istanbul ignore next
    if (typeof disabled !== 'boolean')
      return err(
        'The disable option in the PWA manifest options must be a boolean.'
      );
    if (disabled) return;

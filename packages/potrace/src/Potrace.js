...
Potrace.prototype = {
  ...
  _processLoadedImage: function(image, w, h) {
    var bitmap = new Bitmap(w, h);

    for (var i = 0; i < image.length; i++) {
      var idx = i * 4;
      var opacity = image[idx + 3] / 255,
          r = 255 + (image[idx + 0] - 255) * opacity,
          g = 255 + (image[idx + 1] - 255) * opacity,
          b = 255 + (image[idx + 2] - 255) * opacity;
      bitmap.data[i] = utils.luminance(r, g, b);
    };
   ...
   * Reads given image.
   *
   * @param {string|Buffer} target Image source.
   * @param {Number} w Image width
   * @param {Number} h Image height
   */
  loadImage: function(target, w, h) {
    this._processLoadedImage(target, w, h)
  },
